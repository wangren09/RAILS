...
#             query_object.build(n_trees=100, n_jobs=-1)
        ...
        query_object.load('class'+str(class_label)+'l'+str(self.hidden_layer+1)+'.ann')
        #########load end
...
#             self._build_class_query_object(xh_orig, class_label=i).save('class'+str(i)+'l'+str(self.hidden_layer+1)+'.ann')
        ########save end
        
        ########load start
        xh_orig = self._hidden_repr_mapping(self.x_orig[:2], query=True).detach().cpu().numpy()
        ########load end
    ...
    def _query_nns_ind(self, Q):
        ...
        if self.n_class:
            ...
            #rel_ind = [query_obj.get_nns_by_vector(Q, self.n_neighbors, search_k=-1, include_distances=False) for query_obj in self._query_objects]
    ...
    def clonal_expansion(self, ant, y_ant=None):
        ...
        tend = time.time()
        print('{} seconds cc'.format(tend - t2))
        if self.keep_memory:
