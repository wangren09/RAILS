...
class AISE:
    ...
    def __init__(self, x_orig, y_orig, hidden_layer=None, model=None, input_shape=None, device=torch.device("cuda"),
                 ...
                 mut_range=(.005, .015), mut_prob=(.05, .15), mut_mode="crossover",
        ...
        self.x_orig = x_orig
        self.x_orig = self._hidden_repr_mapone(self.x_orig.detach())
        self.y_orig = y_orig
        ...
        if self.hidden_layer is not None:
            ...
            for i in range(0, x.size(0), batch_size):
                ...
                with torch.no_grad():
                    ...
                    if self.hidden_layer == 1 or self.hidden_layer == 2:
                        xh = xh[:,::1]
                    else:
                        xh = xh[:,::1]
                    if self.normalize:
    ...
    def _hidden_repr_mapone(self, x, batch_size=2048, query=True):
        """
        transform b cells and antigens into inner representations of AISE
        """
        ...
        if self.hidden_layer is not None:
            ...
            for i in range(0, x.size(0), batch_size):
                ...
                with torch.no_grad():
                    if query:
                        xh = self.model.truncated_forward(1)(xx.to(self.device)).detach().cpu()
                    else:
                        xh = self.model.truncated_forward(1)(xx.to(self.device))
#                     if self.avg_channel:
#                         xh = xh.sum(dim=1)
#                     xh = xh.flatten(start_dim=1)
#                     if self.hidden_layer == 1 or self.hidden_layer == 2:
...
#                         xh = xh / xh.pow(2).sum(dim=1, keepdim=True).sqrt()
    ...
    def clonal_expansion(self, ant, y_ant=None):
        ...
        ant = self._hidden_repr_mapone(ant.detach())
