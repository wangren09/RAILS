...
def neg_l2_dist(x, y):
    return -(x - y).pow(2).sum(dim=-1) // 2#.sqrt()
...
class AISE:
    ...
    def generate_b_cells(self, ant, ant_tran, nbc_ind, y_ant=None):
        ...
        if self.return_log:
            ...
            ant_logs = None
        n_population = self.n_class * self.n_neighbors

        head_shape = (self.n_class, n_population // self.n_class)
        # static index
        static_index = torch.arange(n_population).reshape(head_shape).to(self.device)
        ...
        for n in range(ant.size(0)):
            ...
            if self.requires_init:
                ...
                n_population = self.n_class * self.n_neighbors
                curr_gen = curr_gen.repeat_interleave(n_population // (self.n_class * self.n_neighbors), dim=0)
            ...
            beta = [[random.uniform(0,1) for i in range(self.n_neighbors)] for j in range(self.n_class)]
...
#                     true_class_fitness = fitness_score[y_ant[n]].sum().item()
            ...
            eta = 1
            pt = 0.5*torch.ones(n_population)#.repeat_interleave(n_population, dim=0)
            L = 128*8*8
            gamma =  0.5
            delta_sqr = 0.01
            beta_store = torch.empty((100,128,8,8), device=self.device)
            for i in range(self.max_generation):
                eta_new = (i*1.0+1)**(0.5)
                beta = 1/(1+eta_new) * (1+(1-pt)*eta)
#                 print(beta)
#                 print(curr_gen.size())
                for j in range(100):
                    beta_store[j,:,:,:] = beta[j]
                curr_gen = curr_gen * beta_store
                s_sqr = curr_gen.pow(2).sum(dim=(1,2,3))
                curr_repr = self._hidden_repr_mapping(curr_gen).reshape(head_shape+(-1,))
                fitness_score = self.fitness_func(ant_tran[n].to(self.device), curr_repr.to(self.device))
                rho = (1+eta)**2 / (1+eta_new)**2
                if i == 0:
                    rt = -fitness_score.flatten().cpu()
                func = lambda pt_new: pt.cpu().numpy()*rho**(-gamma)*(1 - pt_new*(rt.numpy() - s_sqr.cpu().numpy()) / (rt.numpy() + delta_sqr))**(-gamma) / \
                (1 + pt.cpu().numpy()*rho**(-gamma)*(1 - pt_new*(rt.numpy() - s_sqr.cpu().numpy()) / (rt.numpy() + delta_sqr))**(-gamma)) - pt_new
                pt_solution = fsolve(func, 0.5*torch.ones(n_population))
#                 print(pt_solution)
                pt = pt_solution
                pt = torch.Tensor(pt)
                rt_new = (1 - pt)*rt.cpu().numpy() + pt*s_sqr.cpu().numpy()
                rt = rt_new
        ...
        return None, None, None, pla_labs, None
